...
from dfg.datatype.WaitTime import WaitTime
    ...
    def __call__(self, *args, **kwargs) -> list[Eatery]:
        if "transactions" not in self.cache:
            ...
            while date <= kwargs.get("end"):
                ...
                transaction_avg_counts = TransactionHistoryStore.objects.filter(canonical_date__in=past_days) \
        ...
        for date in self.cache["transactions"]:
            ...
            date_wait_times = WaitTimes.generate_eatery_wait_times_by_day(self.eatery_id, date, eatery_transaction_avgs, kwargs.get("tzinfo"))
