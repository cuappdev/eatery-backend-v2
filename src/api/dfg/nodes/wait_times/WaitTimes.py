...
class WaitTimes(DfgNode):
    ...
    def __call__(self, *args, **kwargs) -> list[Eatery]:
        if "transactions" not in self.cache:
            ...
            while date <= kwargs.get("end"):
                ...
                transaction_avg_counts = (
                    TransactionHistoryStore.objects.filter(canonical_date__in=past_days)
        ...
        for date in self.cache["transactions"]:
            eatery_transaction_avgs = [
                transaction_avg
                for transaction_avg in self.cache["transactions"][date]
                if transaction_avg["eatery_id"] == self.eatery_id.value
            ]
            date_wait_times = WaitTimes.generate_eatery_wait_times_by_day(
                self.eatery_id, date, eatery_transaction_avgs, kwargs.get("tzinfo")
            )
            if date_wait_times is not None:
                eatery_wait_times.append(date_wait_times)
    ...
    def generate_eatery_wait_times_by_day(
        eatery_id: EateryID, date: date, transactions: list, tzinfo: pytz.tzinfo
    ) -> WaitTimesDay:
        wait_times_data = []
        ...
        for index in reversed(range(0, len(transactions))):
            ...
            how_long_ago_guest_arrival = (
                base_times[1]
                + line_decrease_times[1] * transactions[index]["transaction_avg"]
            )
            ...
            if prev_bucket_guest_arrival > 9:
                ...
                customers_waiting_in_line[prev_bucket_guest_arrival] += transactions[
                    index
                ]["transaction_avg"]
                num_customers = customers_waiting_in_line.pop(0)
                wait_time_low = int(
                    base_times[0] + line_decrease_times[0] * num_customers
                )
                wait_time_expected = int(
                    base_times[1] + line_decrease_times[1] * num_customers
                )
                wait_time_high = int(
                    base_times[2] + line_decrease_times[2] * num_customers
                )

                customers_waiting_in_line.append(0.0)
                block_end_time = transactions[index]["block_end_time"]
                timestamp = int(
                    combined_timestamp(date, block_end_time, tzinfo) - 5 * 60 / 2
                )
                wait_times_data.insert(
                    0,
                    WaitTime(
                        timestamp=timestamp,
