...
class OverrideEvent:
    ...
    # Pass in [exists = false] to remove events with the same description and canonical_date from occuring
    def __init__(
            self,
            description: str,
            canonical_date: date,
...
def filter_range(events: list[OverrideEvent], tzinfo: Optional[pytz.timezone], start: Optional[date], end: Optional[date]):
